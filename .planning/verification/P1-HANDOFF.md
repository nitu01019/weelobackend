...
| t1-3-comments-customer (customer-app) | 5 | L1 | `phase-p1/t1-3-legacy-fallback-analytics` (nitu01019/weelo; base = `main`, no `main-new` on that repo) | _PR TBD (human opens)_ | `e3fbfe7` (clean cherry-pick; pre-cleanup `571ea44` carried 3 unintended contract commits F-C-52/63/82 that were stripped) | L1 analytics event only (Crashlytics wrapper, no new SDK) | _awaiting /review output; 8/8 tests green per team-lead_ | completed |
...
- **Two separate PRs (one backend, one customer-app):**
  - **Backend** (L4, M9): branch `phase-p1/t1-3-comments-m9-cleanup`, **rebased onto `main-new` and force-pushed-with-lease** (merge-base verified = `63802612` = `main-new` tip). Origin HEAD = **`b8b3298e`** over `16ce444c`. Supersedes a pre-rebase SHA `90ec3be7` (that SHA stayed in T1.3's intermediate handoff messages but is not on origin). Tests: new `src/__tests__/p1-m9-fcm-click-action.test.ts` with 3 passing tests. Grep verification under `src/` non-test dirs: zero `FLUTTER_NOTIFICATION_CLICK` matches AND zero `clickAction` matches (tests reference the symbols only to assert their absence). **Rebase conflict resolution notes from T1.3:** (1) `progressive-radius-matcher.ts` — `main-new` had the "Fix H-X2 / Unified 6-step" comment on `PROGRESSIVE_RADIUS_STEPS`; T1.3 kept that scaffolding and nested the corrected H3 docstring + h3geo.org citation inside, no runtime change; (2) `fcm.service.ts` — `main-new`'s `buildMessage()` added an `isFullScreen` visibility conditional not on old `main`, so T1.3 kept the visibility-spread semantics and dropped ONLY the `clickAction` field, preserving the "dropped legacy flutter click-action" annotation; (3) `h3-geo-index.service.ts` — auto-merged cleanly. **Additional rebase-surfaced fact:** `main-new` renamed the NEW_BROADCAST channel from `"broadcasts"` to `"broadcasts_v2"` at `fcm.service.ts:752`; T1.3's `b8b3298e` updates the jest expectation to match the new string, 3/3 tests still pass. L4 scope: comment rewrites at `progressive-radius-matcher.ts:66-68` and `h3-geo-index.service.ts:414` + `:418` (flat-direction step ~0.798 km; 0.461 km is the circumradius). PR body file prepared by T1.3 at `/private/tmp/weelo-p1-t1-3/.planning/verification/P1-T1-3-PR-BODY-backend.md`.
  - **Customer-app** (L1): branch `phase-p1/t1-3-legacy-fallback-analytics` in `nitu01019/weelo`, **base = `main`** (customer-app repo does not have a `main-new` branch; team-lead's 2026-04-19 base-branch correction was backend-only). Head SHA **`e3fbfe7`** — this is a **clean cherry-pick** of T1.3's L1 commit onto origin/main. The earlier `571ea44` reference was pre-cleanup; it accidentally carried three contract commits (F-C-52, F-C-63, F-C-82) that had ridden along from an off-HEAD branch point. T1.3 reset to `origin/main` and cherry-picked only the L1 commit. Clean PR = exactly 3 files: `BookingApiRepository.kt` (+11), `BookingTelemetry.kt` (+88 new), `BookingTelemetryTest.kt` (+152 new). Tests: 8/8 green. Event name `booking_legacy_fallback_invoked` with params `{endpoint, primary_code, primary_error_code}`. Sink is **FirebaseCrashlytics** (pre-existing dep) via `FirebaseCrashlytics.getInstance().log` + `setCustomKey`. App has no `firebase-analytics` dependency in `build.gradle`. Event fires only in the `createOrder` fallback branch (line 587-590) when the primary response is 400/404/422/501. No new SDK, no new permissions. PR body file prepared by T1.3 at `/private/tmp/weelo-p1-t1-3/.planning/verification/P1-T1-3-PR-BODY-customer-app.md`.
