 ...
 *   The canonical runtime cleanup path is the `startCleanupJob` method on
 *   `truckHoldService` in truck-hold.service.ts (the monolith) — invoked from
 *   its constructor and wired into the durable HOLD_EXPIRY queue (F-A-77).
 *   The production `setInterval` is owned by the monolith; this module is
 *   retained solely because multiple test files use `fs.readFileSync` on this
 *   file's source text AND jest.mock/require on its exported symbols (see
 *   audit d1-rajat.md — 14 test files, ~200 assertions). Phase 4 will
