...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Extract and VALIDATE idempotency key — required for safe retries.
      // Must be a UUID generated by the client per booking attempt.
      // Without this, retries after the 3s debounce window can create duplicates.
      ...
      if (!idempotencyKey || !UUID_REGEX.test(idempotencyKey)) {
        ...
        res.status(400).json({
          ...
          error: {
            ...
            message: 'X-Idempotency-Key header is required and must be a valid UUID (e.g. uuid/v4).'
          }
        });
        return;
      }

      logger.debug(`🔑 Idempotency key validated: ${idempotencyKey.substring(0, 8)}...`);
