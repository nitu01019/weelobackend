...
class OrderService {
     ...
     */
    async createOrder(customerId, customerPhone, data) {
        ...
        logger_service_1.logger.info(`║  🚛 NEW ORDER REQUEST                                        ║`);
        logger_service_1.logger.info(`╠══════════════════════════════════════════════════════════════╣`);
        logger_service_1.logger.info(`║  Order ID: ${orderId}`);
        logger_service_1.logger.info(`║  Customer: ${customerName} (${customerPhone})`);
        logger_service_1.logger.info(`║  Total Trucks: ${totalTrucks}`);
        logger_service_1.logger.info(`║  Total Amount: ₹${totalAmount}`);
        logger_service_1.logger.info(`║  Truck Types: ${data.trucks.map(t => `${t.quantity}x ${t.vehicleType} ${t.vehicleSubtype}`).join(', ')}`);
        logger_service_1.logger.info(`╚══════════════════════════════════════════════════════════════╝`);
        // ==========================================================================
        // STEP 1: Create parent Order record
        // ==========================================================================
        const order = db_1.db.createOrder({
            id: orderId,
            customerId,
            customerName,
            customerPhone,
        ...
        const createdRequests = db_1.db.createTruckRequestsBatch(truckRequests);
        // ==========================================================================
        ...
        // ==========================================================================
        const broadcastSummary = await this.broadcastToTransporters(order, groupedRequests, data.distanceKm);
        // ==========================================================================
        // STEP 5: Handle case when no transporters found
        // ==========================================================================
        if (broadcastSummary.totalTransportersNotified === 0) {
            logger_service_1.logger.warn(`⚠️ NO TRANSPORTERS FOUND for any truck type in order ${orderId}`);
            // Update order status
            db_1.db.updateOrder(orderId, { status: 'expired' });
            // Update all requests to expired
            db_1.db.updateTruckRequestsBatch(createdRequests.map(r => r.id), { status: 'expired' });
            // Notify customer
            ...
            return {
                ...
                truckRequests: createdRequests.map(r => ({ ...r, status: 'expired' })),
                broadcastSummary,
                timeoutSeconds: 0
            };
        }
        ...
        logger_service_1.logger.info(`✅ Order ${orderId} created in ${processingTime}ms`);
        logger_service_1.logger.info(`   - ${totalTrucks} truck requests created`);
        logger_service_1.logger.info(`   - ${broadcastSummary.totalTransportersNotified} transporters notified`);
        return {
            order,
     ...
     */
    expandTruckSelections(orderId, selections) {
        const requests = [];
        let requestNumber = 1;
     ...
     */
    groupRequestsByVehicleType(requests) {
     ...
     */
    async broadcastToTransporters(order, groupedRequests, distanceKm) {
        const allTransporterIds = new Set();
        const groupSummaries = [];
        // Process each vehicle type group
        for (const group of groupedRequests) {
            // Find transporters with this vehicle type
            const transporterIds = db_1.db.getTransportersWithVehicleType(group.vehicleType, group.vehicleSubtype);
            ...
            db_1.db.updateTruckRequestsBatch(requestIds, { notifiedTransporters: transporterIds });
            ...
            if (transporterIds.length > 0) {
                ...
                for (const transporterId of transporterIds) {
                    ...
                    logger_service_1.logger.info(`📢 Notified: ${transporter?.name || transporterId} for ${group.vehicleType} ${group.vehicleSubtype} (${group.requests.length} trucks)`);
                }
            }
            else {
                logger_service_1.logger.warn(`⚠️ No transporters found for ${group.vehicleType} ${group.vehicleSubtype}`);
            }
        }
        return {
     ...
     */
    startOrderTimeout(orderId, customerId) {
        ...
        logger_service_1.logger.info(`⏱️ Timeout timer started for order ${orderId} (${ORDER_CONFIG.TIMEOUT_MS / 1000}s)`);
    }
    /**
     * Handle order timeout
     */
    async handleOrderTimeout(orderId, customerId) {
        const order = db_1.db.getOrderById(orderId);
        if (!order) {
            logger_service_1.logger.warn(`Order ${orderId} not found for timeout handling`);
            return;
        }
        // Skip if already completed or cancelled
        if (['fully_filled', 'completed', 'cancelled'].includes(order.status)) {
            logger_service_1.logger.info(`Order ${orderId} already ${order.status}, skipping timeout`);
            this.clearOrderTimers(orderId);
            return;
        }
        logger_service_1.logger.info(`⏰ TIMEOUT: Order ${orderId} expired`);
        // Get unfilled requests
        const requests = db_1.db.getTruckRequestsByOrder(orderId);
        ...
        db_1.db.updateTruckRequestsBatch(unfilledRequests.map(r => r.id), { status: 'expired' });
        ...
        const allNotifiedTransporters = new Set();
     ...
     */
    startCountdownNotifications(orderId, customerId) {
    ...
    cancelOrderTimeout(orderId) {
        this.clearOrderTimers(orderId);
        logger_service_1.logger.info(`⏱️ Timeout cancelled for order ${orderId}`);
    }
    // ==========================================================================
     ...
     */
    async acceptTruckRequest(requestId, transporterId, vehicleId, driverId) {
        const startTime = Date.now();
        // STEP 1: Validate request exists and is available
        const request = db_1.db.getTruckRequestById(requestId);
        if (!request) {
            throw new error_types_1.AppError(404, 'REQUEST_NOT_FOUND', 'Truck request not found');
        }
        if (request.status !== 'searching') {
            // Request already taken - send immediate feedback
            throw new error_types_1.AppError(400, 'REQUEST_ALREADY_TAKEN', 'This truck request was just taken by another transporter. Check for remaining trucks.');
        }
        ...
        if (vehicle.vehicleType !== request.vehicleType || vehicle.vehicleSubtype !== request.vehicleSubtype) {
            throw new error_types_1.AppError(400, 'VEHICLE_TYPE_MISMATCH', `Your vehicle (${vehicle.vehicleType} ${vehicle.vehicleSubtype}) doesn't match the request (${request.vehicleType} ${request.vehicleSubtype})`);
        }
        // Get transporter and driver info
        const transporter = db_1.db.getUserById(transporterId);
        const driver = driverId ? db_1.db.getUserById(driverId) : null;
        // STEP 2: Update the truck request IMMEDIATELY (atomic operation)
        const tripId = (0, uuid_1.v4)();
        const updatedRequest = db_1.db.updateTruckRequest(requestId, {
            status: 'assigned',
        ...
        const order = db_1.db.getOrderById(request.orderId);
        if (!order) {
            logger_service_1.logger.error(`Order ${request.orderId} not found for request ${requestId}`);
        ...
        db_1.db.updateOrder(request.orderId, {
            trucksFilled: newFilled,
            status: newStatus
        });
        // Get remaining searching requests for this vehicle type
        const allRequests = db_1.db.getTruckRequestsByOrder(request.orderId);
        const remainingRequests = allRequests.filter(r => r.status === 'searching' &&
            r.vehicleType === request.vehicleType &&
            r.vehicleSubtype === request.vehicleSubtype);
        logger_service_1.logger.info(`╔══════════════════════════════════════════════════════════════╗`);
        logger_service_1.logger.info(`║  ✅ TRUCK ACCEPTED                                           ║`);
        logger_service_1.logger.info(`╠══════════════════════════════════════════════════════════════╣`);
        logger_service_1.logger.info(`║  Request: ${requestId}`);
        logger_service_1.logger.info(`║  Transporter: ${transporter?.name || transporterId}`);
        logger_service_1.logger.info(`║  Vehicle: ${vehicle.vehicleNumber} (${vehicle.vehicleType} ${vehicle.vehicleSubtype})`);
        logger_service_1.logger.info(`║  Progress: ${newFilled}/${order.totalTrucks} trucks filled`);
        logger_service_1.logger.info(`║  Remaining (same type): ${remainingRequests.length}`);
        logger_service_1.logger.info(`║  Processing time: ${Date.now() - startTime}ms`);
        logger_service_1.logger.info(`╚══════════════════════════════════════════════════════════════╝`);
        // STEP 4: Send INSTANT confirmation to accepting transporter
        (0, socket_service_1.emitToUser)(transporterId, socket_service_1.SocketEvent.ACCEPT_CONFIRMATION, {
            success: true,
            requestId,
            orderId: order.id,
        ...
        const notifiedTransporters = new Set();
        ...
        for (const otherTransporterId of notifiedTransporters) {
            if (otherTransporterId !== transporterId) {
                ...
                (0, socket_service_1.emitToUser)(otherTransporterId, socket_service_1.SocketEvent.REQUEST_NO_LONGER_AVAILABLE, {
                    orderId: order.id,
                    requestId,
                ...
                (0, socket_service_1.emitToUser)(otherTransporterId, socket_service_1.SocketEvent.TRUCKS_REMAINING_UPDATE, {
                    orderId: order.id,
                    vehicleType: request.vehicleType,
                    vehicleSubtype: request.vehicleSubtype,
        ...
        (0, socket_service_1.emitToUser)(order.customerId, socket_service_1.SocketEvent.TRUCK_ASSIGNED, {
            orderId: order.id,
            requestId,
        ...
        if (newStatus === 'fully_filled') {
            ...
            (0, socket_service_1.emitToUser)(order.customerId, socket_service_1.SocketEvent.BOOKING_FULLY_FILLED, {
                orderId: order.id,
                totalTrucks: order.totalTrucks,
            ...
            for (const tid of notifiedTransporters) {
                if (tid !== transporterId) {
                    (0, socket_service_1.emitToUser)(tid, socket_service_1.SocketEvent.ORDER_STATUS_UPDATE, {
                        orderId: order.id,
                        status: 'fully_filled',
            ...
            logger_service_1.logger.info(`🎉 ORDER ${order.id} FULLY FILLED!`);
        }
        return updatedRequest;
    }
    // ==========================================================================
     ...
     */
    async getOrderWithRequests(orderId, userId, userRole) {
        const order = db_1.db.getOrderById(orderId);
        if (!order) {
            throw new error_types_1.AppError(404, 'ORDER_NOT_FOUND', 'Order not found');
        }
        // Access control
     ...
     */
    async getActiveTruckRequestsForTransporter(transporterId) {
        const requests = db_1.db.getActiveTruckRequestsForTransporter(transporterId);
        // Group by order for better UI
        const orderMap = new Map();
        for (const request of requests) {
            if (!orderMap.has(request.orderId)) {
                orderMap.set(request.orderId, {
                    order: db_1.db.getOrderById(request.orderId),
                    requests: []
                });
            }
    ...
    async getCustomerOrders(customerId, page = 1, limit = 20) {
        const orders = db_1.db.getOrdersByCustomer(customerId);
        ...
        const ordersWithSummary = paginatedOrders.map(order => {
            const requests = db_1.db.getTruckRequestsByOrder(order.id);
            return {
                ...order,
                requestsSummary: {
