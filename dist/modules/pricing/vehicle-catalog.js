...
exports.DISTANCE_SLABS = exports.VEHICLE_CATALOG = void 0;
exports.getDistanceSlabMultiplier = getDistanceSlabMultiplier;
exports.getVehicleConfig = getVehicleConfig;
exports.getSubtypeConfig = getSubtypeConfig;
exports.findSuitableVehicles = findSuitableVehicles;
 ...
 */
function getDistanceSlabMultiplier(distanceKm) {
    for (const slab of exports.DISTANCE_SLABS) {
...
function getVehicleConfig(vehicleType) {
    return exports.VEHICLE_CATALOG[vehicleType.toLowerCase()] || null;
}
/**
 * Get subtype config
 */
function getSubtypeConfig(vehicleType, subtype) {
 ...
 */
function findSuitableVehicles(cargoWeightKg, preferredType) {
    const results = [];
    // Search through all vehicle types
    const typesToSearch = preferredType
        ? [preferredType.toLowerCase()]
        : Object.keys(exports.VEHICLE_CATALOG);
    for (const vehicleType of typesToSearch) {
        const config = exports.VEHICLE_CATALOG[vehicleType];
        if (!config)
            continue;
...
function getAllVehicleTypes() {
    return Object.values(exports.VEHICLE_CATALOG).map(config => {
...
//# sourceMappingURL=vehicle-catalog.js.map